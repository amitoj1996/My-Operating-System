...
void AdvancedTechnologyAttachment::Identify()
{
  ...
  if(status == 0xFF)
    return ;//means that there is no device on that bus
  ...
  if(status == 0x00)
    ...
    for(uint16_t i = 0;i<256;i++)//should have been 512 to read 512 sector bytes but the data we are reading is 2 bytes each so 256
    {
...
void AdvancedTechnologyAttachment::Read28(uint32_t sector, uint8_t* data, int count)
{
  ...
  printf("Reading from ATA: ");
    ...
    for(uint16_t i = 0;i<count;i+=2)//should have been 512 to read 512 sector bytes but the data we are reading is 2 bytes each so 256
    {
